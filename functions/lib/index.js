...
// We don't need Vite in Firebase Functions.
// We just need the API routes.
...
async function fetchSerperShopping(query) {
    if (serperCache.has(query)) {
        return serperCache.get(query);
    }
    // Fallback to process.env for local, or functions.config() in prod
    const apiKey = process.env.SERPER_API_KEY || (functions.config().skey && functions.config().skey.key);
    if (!apiKey) {
...
async function resolveDirectLink(item) {
...
async function wrapWithCuelinks(originalUrl) {
    if (!originalUrl)
        return originalUrl;
    try {
...
app.post("/curate", async (req, res) => {
    ...
    try {
        ...
        let occType = "casual";
        if (occLower.includes("office") || occLower.includes("work") || occLower.includes("formal"))
            occType = "office";
        if (occLower.includes("festive") || occLower.includes("wedding") || occLower.includes("traditional"))
            occType = "festive";
        if (occLower.includes("gym") || occLower.includes("active") || occLower.includes("sport"))
            occType = "gym";
        ...
        if (topwearClean.length === 0 || bottomwearClean.length === 0) {
            return res.status(500).json({ error: "Could not find enough matching products from search." });
        }
        const outfits = [];
        ...
        for (let i = 0; i < 3; i++) {
            if (!validCombinations[i % validCombinations.length])
                break;
...
// Export the Express API wrapped as a Firebase Function
// Note: we export 'api' so it matches the rewrite rule in firebase.json {"function": "api"}
